using System;
/*Every object of the SimpleProjectile-class is made to represent a projectile of some kind.
 *It is, as the name suggests, simple. It assumes that the only force acting on it during flight
 *is the force of gravity. It extends ODE and can by modularity be extended/evolved further.*/
...
public class SimpleProjectile : ODE
{
  ...
  public SimpleProjectile(double x0, double y0, double z0, 
     ...
     *values into the s field and q array from the ODE class.*/
    this.S = time;
    SetQ(vx0,0);
    SetQ(x0, 1);
    SetQ(vy0,2);
  ...
  //These methods return the location, velocity, and time values.
  public double GetVx() {
    return GetQ(0);
  }
