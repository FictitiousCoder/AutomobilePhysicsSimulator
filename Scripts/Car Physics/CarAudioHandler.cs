...
public class CarAudioHandler : MonoBehaviour {
	 ...
	 * at different RPM and Throttle states.*/
	...
	public AudioClip[] crashClips = null;												// Audio clips for crashing
    ...
    private Car car = null;                                                       				// Reference to car we are controlling
	...
	void Update () {
		...
		if (startedSound) {
			...
			float pitch = ULerp (lowPitchMin,lowPitchMax,(float)(car.OmegaE/8000));
			...
			if (engineSoundStyle == EngineAudioOptions.Simple)
			{
				...
				float accFade = Mathf.Abs ( (float)(car.GetVx()*0.03) );
				float decFade = 1-accFade;
				
                // get the high fade value based on the cars revs
				float highFade = Mathf.InverseLerp(0.2f,0.8f,(float)(car.OmegaE/8000));
			...
			/*
			skidSource.volume = Mathf.Clamp01((float)car.GetVy()*1000 * 3 - 1);
			skidSource.pitch = Mathf.Lerp (0.8f, 1.3f, (float)car.GetVy()*1000);
	...
	public void CrashNoise(int type)
	{
		AudioSource.PlayClipAtPoint(crashClips[type], transform.position);
