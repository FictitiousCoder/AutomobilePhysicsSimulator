using System;
/*The DragProjectile class extends the SimpleProjectile class in order to add
 *the effects of aerodynamic drag to the projectile-object.*/
...
public class DragProjectile : SimpleProjectile
{
  ...
  //These properties are used to access the fields declared in the class.
  public double Mass {
    get {
      return mass;
    }
  ...
  public override double[] GetRightHandSide(double s, double[] q, 
	...
	//Declare some convenience variables representing the intermediate values of velocity.
    double vx = newQ[0];
    double vy = newQ[2];
    double vz = newQ[4];
